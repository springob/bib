...
Blockly.JavaScript['controls_repeat_ext'] = function(block) {
  ...
  branch = Blockly.JavaScript.currentBlockTracer(block) + branch;
  branch = Blockly.JavaScript.addLoopTrap(branch, block);
  var code = '';
  code += 'for (let _cnt=0, _end='+repeats+'; _cnt<_end; _cnt++) {\n' +
      branch + '}\n';
  return code;
};
...
Blockly.JavaScript['controls_repeat_until'] = function(block) {
  // Do while/until loop.
  var argument0 = Blockly.JavaScript.valueToCode(block, 'COND', Blockly.JavaScript.ORDER_LOGICAL_NOT) || 'false';
  var branch = Blockly.JavaScript.statementToCode(block, 'DO');
  branch = Blockly.JavaScript.currentBlockTracer(block) + branch;
  branch = Blockly.JavaScript.addLoopTrap(branch, block);
  argument0 = '!' + argument0;
...
Blockly.JavaScript['controls_flow_statements'] = function(block) {
  ...
  if (Blockly.JavaScript.STATEMENT_PREFIX) {
    ...
    xfix += Blockly.JavaScript.injectId(Blockly.JavaScript.STATEMENT_PREFIX, block);
  }
  if (Blockly.JavaScript.STATEMENT_SUFFIX) {
    // Inject any statement suffix here since the regular one at the end
    // will not get executed if the break/continue is triggered.
    xfix += Blockly.JavaScript.injectId(Blockly.JavaScript.STATEMENT_SUFFIX, block);
  }
  if (Blockly.JavaScript.STATEMENT_PREFIX) {
    ...
    if (loop && !loop.suppressPrefixSuffix) {
      ...
      xfix += Blockly.JavaScript.injectId(Blockly.JavaScript.STATEMENT_PREFIX, loop);
    }
  }
...
Blockly.JavaScript['controls_loop_break'] = function(block) {
    return 'break;\n';
}
