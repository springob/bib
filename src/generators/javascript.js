...
goog.provide('Blockly.JavaScript.fincodes');
...
Blockly.JavaScript.scrub_ = function(block, code, opt_thisOnly) {
  ...
  var finCode='';
  if (!block.getParent() && block.hasOwnProperty('isProcedureHead') && block.isProcedureHead) {
...
Blockly.JavaScript.create_symbol_name = function (symbolName) {
    return Blockly.BIB.lib.normalizeSymbolname(symbolName);
    //return symbolName.replace(/\s/g,"_");
}

Blockly.JavaScript.get_datatype_ = function (type) {
    if (type=='Number') type='int';
    if (type=='Colour') type='color_t';
...
Blockly.JavaScript.prefixTrimmedLines = function (lines, prefix) {
    return lines.replace(/^\s+/gm, prefix);
}


Blockly.JavaScript.stringValueToCode = function (block, name, order) {
    var text = Blockly.JavaScript.valueToCode(block, name, order) || '""';
    var types = block.getInputTargetBlock(name).outputConnection.getCheck();
    const relevantTypes = ['String', 'Number', 'Boolean', 'Colour', 'Image'];
    ...
    if (types.length==1) {
        ...
        if (type=='Colour') text = 'stringFromColor('+text+')';
        if (type=='Image') text = 'stringFromBitmap('+text+')';
    }
    return text;
}

Blockly.JavaScript.addLoopTrap = function (branch, block) {
    if (branch=='') {
        branch = 'await bob3.nop();\n';
    }
    return branch;
}

Blockly.JavaScript.currentBlockTracer = function(block) {
    return 'await bob3.traceCurrentBlock(\''+block.id+'\');\n';
}
